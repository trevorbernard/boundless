...
In order to amortize the Boundless' onchain verification cost,
...
For efficient verification of proofs onchain, this process builds a binary Merkle tree of the receipt claims.

## Boundless Market

A coordination and clearing mechanism that connects those requesting proof generation with those providing proving power,
...
- Calculate an appropriate price and lock-in collateral for the request.
...
- A URL where the program with required Image ID's can be retrieved by the prover.
