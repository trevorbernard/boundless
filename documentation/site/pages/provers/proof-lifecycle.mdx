...
| **Ramp-up start**            | Defined as a block number.                                 |
...
| **Lock-in collateral**       | The slashable amount if a prover fails to deliver a proof. |
...
| Ramp-up start            | Latest block number + 3 |
| Length of ramp-up period | 10 blocks               |
| Timeout                  | 100 blocks              |
| Lock-in collateral       | 5 USDC                  |
...
However, locking a request requires the prover to put up collateral, which will be slashed if they fail to deliver a proof before the request timeout. Requestors choose the collateral value according to their application's requirements. A low collateral value may result in a better price, while a higher collateral value decreases the chance an unreliable prover will lock the request. Applications such as fraud proofs may disable lock-in entirely by setting the collateral amount impossibly high (e.g., higher than the total supply of the collateral token).
